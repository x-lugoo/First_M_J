...
int book_page_refresh()
{
	...
	/** As a remained function for add price after the name of product;
	char tmpBuff[100];
	int iPrice = 0;
	**/
	...
	for (i=0;i!=count;++i) {
		...
		if (node) {
			...
			XuiButtonStat stat_common;
			{
				...
				/*** As a remained function for add price after the name of product;
					iPrice = product_node_get_price(node);
					if(iPrice > 0)
					{
						snprintf(tmpBuff,99,"%s",node_get_btn_text(node));
					}
					else
					{
						snprintf(tmpBuff,99,"%s",node_get_btn_text(node));
					}
					stat_common.text=tmpBuff;
				**/
